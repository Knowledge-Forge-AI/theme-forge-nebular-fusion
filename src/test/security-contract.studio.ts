...
describe("Studio package and authority isolation", () => {
  ...
  it("binds invoke handler, app manifest, generated permissions, and main capability to one inventory", async () => {
    ...
    const handlers = [...lib.matchAll(/commands::(?:brand_read|brand_plan|host|selection|design_packet)::([a-z][a-z0-9_]*)/g)].map((match) => match[1]);
    ...
    expect(inventory).toEqual(["studio_brand_read", "studio_brand_plan_start", "studio_brand_plan_cancel", "studio_host_start", "studio_host_status", "studio_select_project", "studio_select_source", "studio_host_shutdown", "studio_design_packet_import", "studio_design_packet_export"]);
  ...
  it("uses the exact minimal CSP and no global or remote navigation authority", async () => {
    const config = JSON.parse(await text("src-tauri/tauri.conf.json")) as {
      app: { windows: Array<{ label: string; url?: string }>; withGlobalTauri: boolean; security: { csp: string } };
    };
